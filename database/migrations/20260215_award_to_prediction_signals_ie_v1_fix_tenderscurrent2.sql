-- Civant: Award-to-Prediction Signals v1 (Ireland only) - Fix 2
-- Reason: In some environments regclass equality checks can behave unexpectedly with
-- mixed-case quoted identifiers; use a text-based guard for "TendersCurrent".
...
begin
  ...
  if v_rel_text = '"TendersCurrent"' or v_rel_text = 'public."TendersCurrent"' then
    return query
    with
    src as (
      select
  ...
  raise exception 'Unsupported source relation for refresh_award_fact_ie: %', v_rel_text;
end;
$$;
