-- Search should return one row per canonical tender, not per raw notice.

create extension if not exists pg_trgm;

create index if not exists idx_canonical_tenders_tenant_sort_ts
  on public.canonical_tenders (tenant_id, last_seen_at desc nulls last, updated_at desc nulls last);

create index if not exists idx_canonical_tenders_tenant_country
  on public.canonical_tenders (tenant_id, country);

create index if not exists idx_canonical_tenders_tenant_source_summary
