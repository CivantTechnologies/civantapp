-- Civant: IE Award-to-Prediction Signals v1
-- Fixes:
-- 1) Buyer fallback: include data->>'client_contracting_authority' when contracting_authority/buyer_name are missing
-- 2) Supplier normalization: correctly strip common legal suffixes (case/punctuation-insensitive via preprocessing)
--
-- Scope: ETENDERS_IE only. Canonical tables unchanged.

begin;

-- Supplier normalization v1 fix:
-- Postgres regex is POSIX-ish; use [[:space:]] instead of \s for portability.
...
as $$
  ...
  s3 as (
    ...
    select trim(regexp_replace(s, '([[:space:]]+(ltd|limited|plc|gmbh|sarl|sl|bv|ag|inc|llc))+$', '', 'g')) as s from s2
  ),
  s4 as (
    select trim(regexp_replace(s, '[[:space:]]+', ' ', 'g')) as s from s3
  )
  select nullif(s, '') from s4;
$$;

-- Buyer fallback fix inside refresh_award_fact_ie():
-- Prefer embedded buyer ids if present; else stable id from best available buyer name field.
...
as $$
    ...
    select
      ...
      coalesce(
        ...
        civant_ie__stable_id(
          ...
          'ETENDERS_IE',
          null,
          nullif(coalesce(d->>'contracting_authority', d->>'buyer_name', d->>'client_contracting_authority'), '')
        )
      ) as buyer_id,
      nullif(
        coalesce(
  ...
  dim_upsert as (
    ...
    from (
      ...
      order by supplier_id, length(supplier_name_raw) desc
    ) ws
