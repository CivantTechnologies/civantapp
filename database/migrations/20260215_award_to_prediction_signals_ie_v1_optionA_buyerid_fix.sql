-- Civant: Award-to-Prediction Signals v1 (Ireland only) - Option A buyer_id stability fix
-- Bug: buyer_id was derived using source_notice_id, which changes per tender, causing buyer_id to be unique per award.
-- Fix: derive buyer_id from stable buyer name (and any real buyer id if present).
