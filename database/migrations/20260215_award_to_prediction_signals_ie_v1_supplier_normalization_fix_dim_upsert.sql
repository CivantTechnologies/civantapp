-- Civant: Supplier normalization v1 fix (Ireland only)
-- Fixes refresh_award_fact_ie() failing with:
--   "ON CONFLICT DO UPDATE command cannot affect row a second time"
-- Root cause: dim_upsert was inserting multiple rows per supplier_id (same normalized name, different raw variants).
-- Fix: dedupe by supplier_id (DISTINCT ON) and use ON CONFLICT DO NOTHING.
...
as $$
  ...
  dim_upsert as (
    ...
    from (
      ...
      -- prefer longer raw examples; otherwise arbitrary but stable enough for v1
