-- Canonical country normalization + ranked search performance guardrails
-- A) Normalize canonical/notices country codes for national/TED coverage
-- B) Reduce ranked search timeout risk with recency expression indexes and safer caps
...
as $$
  with c as (
    select upper(trim(coalesce(p_country, ''))) as code
  )
  select case
    when code = '' then null
    when code in ('IE', 'IRL', 'IRELAND') then 'IE'
    when code in ('FR', 'FRA', 'FRANCE') then 'FR'
    when code in ('ES', 'ESP', 'SPAIN') then 'ES'
    when code in ('GB', 'GBR', 'UK', 'UNITED KINGDOM') then 'GB'
    when code in ('DE', 'DEU', 'GERMANY') then 'DE'
    when code in ('IT', 'ITA', 'ITALY') then 'IT'
    when code in ('PT', 'PRT', 'PORTUGAL') then 'PT'
    when code in ('NL', 'NLD', 'NETHERLANDS') then 'NL'
    when code in ('BE', 'BEL', 'BELGIUM') then 'BE'
    when char_length(code) = 2 then code
    else code
  end
  from c
$$;

create or replace function public.infer_country_from_source(p_source text)
returns text
language sql
immutable
as $$
  select case
    when upper(coalesce(p_source, '')) like '%_IE' then 'IE'
    when upper(coalesce(p_source, '')) like '%_FR' then 'FR'
    when upper(coalesce(p_source, '')) like '%_ES' then 'ES'
    else null
  end
$$;
...
set country = public.normalize_country_code(
  coalesce(
    nullif(n.country, ''),
    public.infer_country_from_source(n.source)
  )
)
...
set country = public.normalize_country_code(
  coalesce(
    nullif(ct.country, ''),
    public.infer_country_from_source(ct.source),
    case
      when exists (
        select 1
        from unnest(coalesce(ct.verification_sources, '{}'::text[])) as s
        where upper(s) = 'PLACSP_ES'
      ) then 'ES'
      when exists (
        select 1
        from unnest(coalesce(ct.verification_sources, '{}'::text[])) as s
        where upper(s) = 'BOAMP_FR'
      ) then 'FR'
      when exists (
        select 1
        from unnest(coalesce(ct.verification_sources, '{}'::text[])) as s
        where upper(s) = 'ETENDERS_IE'
      ) then 'IE'
      else null
    end
  )
...
create index if not exists idx_canonical_tenders_tenant_recency_desc
  on public.canonical_tenders (tenant_id, (coalesce(last_seen_at, updated_at, created_at)) desc);

create index if not exists idx_canonical_tenders_tenant_source_recency_desc
  on public.canonical_tenders (tenant_id, source, (coalesce(last_seen_at, updated_at, created_at)) desc);

create index if not exists idx_canonical_tenders_tenant_country_recency_desc
  on public.canonical_tenders (tenant_id, country, (coalesce(last_seen_at, updated_at, created_at)) desc);

create index if not exists idx_canonical_tenders_tenant_effective_country
  on public.canonical_tenders (
    tenant_id,
    (public.normalize_country_code(coalesce(nullif(country, ''), public.infer_country_from_source(source))))
  );
...
declare
  ...
  v_country text := public.normalize_country_code(p_country);
...
begin
  ...
  if v_source is not null and v_country is not null then
    v_candidate_cap := least(greatest(v_limit * 5, 1500), 7000);
  elsif v_source is not null or v_country is not null then
    v_candidate_cap := least(greatest(v_limit * 8, 2000), 10000);
  elsif v_keyword is not null then
    v_candidate_cap := least(greatest(v_limit * 10, 3000), 10000);
  elsif v_deadline_within is not null then
    v_candidate_cap := least(greatest(v_limit * 8, 2500), 8500);
  else
    v_candidate_cap := least(greatest(v_limit * 10, 3000), 9000);
  end if;
  ...
  scored as (
    select
      ...
      (
        ...
        + case when upper(coalesce(f.source, '')) = 'PLACSP_ES' and f.country = 'ES' then 3 else 0 end
      )::numeric as rel
    from filtered f
  ),
  deduped as (
