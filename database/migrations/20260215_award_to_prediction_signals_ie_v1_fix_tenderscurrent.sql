-- Civant: Award-to-Prediction Signals v1 (Ireland only) - Fix for current staging schema
-- Reason: In this DB, award/tender "notice_type" lives inside public."TendersCurrent".data (jsonb),
-- and public.canonical_tenders does not have a notice_type column. The original v1 selector required
-- a notice_type column and could return NULL.
...
begin
  ...
  if v_rel is not null
    and civant__col_exists(v_rel, 'tender_id')
    and civant__col_exists(v_rel, 'source')
    and civant__col_exists(v_rel, 'tenant_id')
    and civant__col_exists(v_rel, 'published_at')
    and civant__col_exists(v_rel, 'data')
  then
    return v_rel;
  end if;
  return null;
...
begin
  ...
  -- Special-case: public."TendersCurrent" stores notice_type + award-ish fields in jsonb "data".
  if v_rel = to_regclass('public."TendersCurrent"') then
    return query
    with
    src as (
      select
        tc.tenant_id::text as tenant_id,
        tc.source::text as source_system,
        tc.tender_id::text as award_canonical_id,
        lower(coalesce(tc.data->>'notice_type','')) as notice_type,
        tc.data as d,
        tc.published_at
  ...
  -- Fallback: call the original implementation for other relations (if any).
  raise exception 'Unsupported source relation for refresh_award_fact_ie: %', v_rel::text;
end;
$$;
