-- =============================================================================
-- Civant: Buyer Category Stats Recompute V2.1
-- Migration: 20260220_buyer_category_stats_v2_1.sql
-- =============================================================================
-- 
-- CHANGES FROM V1:
-- 1) Cycle interval cap raised from 730 days (2yr) to 2555 days (7yr)
--    to capture multi-year public procurement contracts (3-5yr frameworks).
-- 2) Cadence reliability formula reworked:
--    - Uses coefficient of variation (CV) with a gentler sigmoid curve
--    - Real-world variance (e.g. 300-400 day cycles) no longer tanks the score
--    - Interval count weighted more heavily to reward depth of evidence
-- 3) Sparse-pair safe behavior: no row drops for low-notice buyers.
-- 4) Data coverage formula rebalanced to reward deeper signal history.
-- 5) Guard: rejects unresolved buyer IDs and cluster_unknown at function level.
--
-- ROLLBACK: Re-apply 20260222_buyer_category_stats_backfill_fix_v1.sql
-- =============================================================================

create or replace function public.recompute_buyer_category_stats_v2_1(
  p_tenant_id text,
...
begin
  ...
  -- Minimum gap of 7 days filters out same-batch duplicate publications.
  -- =========================================================================
  cycle_intervals as (
    select gap_days
    from notice_gaps
    where gap_days between 7 and 2555
    union all
    select gap_days
...
-- =============================================================================
-- UPDATE the refresh wrapper to call v2.1
-- =============================================================================
...
-- SELECT proname FROM pg_proc WHERE proname = 'recompute_buyer_category_stats_v2_1';
--
-- Test on a single known Irish buyer:
-- SELECT public.recompute_buyer_category_stats_v2_1(
--   'civant_default',
--   (SELECT DISTINCT buyer_entity_id FROM signals
--    WHERE tenant_id='civant_default' AND region='IE'
--      AND buyer_entity_id NOT LIKE 'unresolved:%'
--    LIMIT 1),
--   (SELECT DISTINCT cpv_cluster_id FROM signals
--    WHERE tenant_id='civant_default' AND region='IE'
...
-- SELECT * FROM buyer_category_stats
-- WHERE tenant_id = 'civant_default' AND region = 'IE'
