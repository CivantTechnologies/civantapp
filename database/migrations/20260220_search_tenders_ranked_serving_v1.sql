-- Stop-the-bleeding: switch search_tenders_ranked to notices_search_current serving table
-- Keeps a legacy function for safe fallback when serving table is empty.

create or replace function public.search_tenders_ranked_legacy(
  p_tenant_id text,
...
begin
  ...
  with base as (
    ...
    where nsc.tenant_id = v_tenant_id
      ...
      and (
        v_source is null
        or upper(coalesce(nsc.source, '')) = v_source
        or (
          nsc.verification_sources is not null
          and exists (
            select 1
            from unnest(nsc.verification_sources) as s
            where upper(s) = v_source
          )
        )
      )
      ...
      and (
        v_published_cutoff is null
        or coalesce(nsc.last_seen_at, nsc.updated_at, nsc.last_updated_at) >= v_published_cutoff
      )
    order by coalesce(nsc.last_seen_at, nsc.updated_at, nsc.last_updated_at) desc nulls last
    limit v_candidate_cap
  ),
  filtered as (
    select b.*
