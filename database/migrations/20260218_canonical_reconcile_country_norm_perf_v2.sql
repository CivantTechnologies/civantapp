-- Reconcile runtime fix: use indexed source/country predicates now that countries are normalized.
...
begin
  ...
  if p_national_sources is null or cardinality(p_national_sources) = 0 then
    if v_country = 'IE' then
      ...
      select coalesce(array_agg(distinct n.source), '{}'::text[])
  ...
  loop
    ...
    v_notice_country := coalesce(r.country, v_country);
    ...
    with seeded_candidates as (
      ...
      where ct.tenant_id = v_tenant_id
        ...
        and ct.source = any(v_national_sources)
        and (v_notice_country is null or ct.country is null or ct.country = v_notice_country)
        and (
          (
        ...
        and (
          ...
          or abs(ct.publication_date - r.publication_date) <= 120
        )
    ...
    candidates as (
      select
        ...
        (
          ...
          + case
              when v_notice_country is not null and v_notice_country <> '' and ct.country = v_notice_country
                then 0.05
              else 0
            end
          + case
