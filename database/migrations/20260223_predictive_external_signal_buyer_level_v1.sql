-- =============================================================================
-- Civant: External signal selection includes buyer-level signals across clusters
-- Migration: 20260223_predictive_external_signal_buyer_level_v1.sql
-- =============================================================================
-- Change:
--   In public.compute_prediction_for_pair_v1, external signal selection now treats
--   cpv_cluster_id NULL or 'cluster_unknown' as buyer-level signals that apply to
--   all buyer pairs, while cluster-specific signals still require exact pair cluster match.
--
--   Existing scoring caps remain unchanged.
-- =============================================================================
...
-- Baseline (historical reference): with_external ~= 0.17%
-- Rerun incremental for 30 days:
--   select run_id, status, pairs_processed, error_message
--   from public.run_predictive_engine_v1('civant_default', 'incremental', 'v1.0.0', now() - interval '30 days', 50000, true);
--
-- Verify with_external percentage rose materially:
--   select
--     count(*) filter (where ps.external_signal_score > 0) as with_external,
--     count(*) as total,
--     round(100.0 * count(*) filter (where ps.external_signal_score > 0) / nullif(count(*), 0), 2) as with_external_pct
--   from public.predictions_current pc
--   join public.prediction_scorecard ps
--     on ps.tenant_id = pc.tenant_id
--    and ps.prediction_id = pc.prediction_id
