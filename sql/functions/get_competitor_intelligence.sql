...
begin
  ...
  -- For short terms (<=4 chars), use word-boundary regex to avoid false positives
  ...
  from (
    select supplier_name_raw from award_fact_ie
    where tenant_id = p_tenant_id
      and CASE WHEN v_regex IS NOT NULL THEN lower(coalesce(supplier_name_raw, '')) ~ v_regex
          ELSE lower(coalesce(supplier_name_raw, '')) like v_search END
    union all
    select supplier_name_raw from award_fact_fr
    where tenant_id = p_tenant_id
      and CASE WHEN v_regex IS NOT NULL THEN lower(coalesce(supplier_name_raw, '')) ~ v_regex
          ELSE lower(coalesce(supplier_name_raw, '')) like v_search END
    union all
    select supplier_name_raw from award_fact_es
    where tenant_id = p_tenant_id
      and CASE WHEN v_regex IS NOT NULL THEN lower(coalesce(supplier_name_raw, '')) ~ v_regex
          ELSE lower(coalesce(supplier_name_raw, '')) like v_search END
  ) s;
    ...
    select
      ...
      (
        ...
        where a2.buyer_id = a.buyer_id
          and CASE WHEN v_regex IS NOT NULL THEN lower(coalesce(a2.supplier_name_raw, '')) ~ v_regex
              ELSE lower(coalesce(a2.supplier_name_raw, '')) like v_search END
      ) as repeat_wins,
      case
