...
# Civant: Spain external funding signals rollout (datos.gob.es)
# Usage:
#   ./scripts/rollout-external-signals-es.sh TENANT_ID [START_DATE] [DRY_RUN]
...
FETCH_SCRIPT="${REPO_ROOT}/scripts/funding/funding-es-datosgob-incremental.mjs"
QA_SQL="${REPO_ROOT}/scripts/qa-external-signals-es.sql"
TMP_DIR="${TMPDIR:-/tmp}"
TSV_FILE="$(mktemp "${TMP_DIR%/}/civant_extsig_es_XXXXXX" 2>/dev/null || mktemp -t civant_extsig_es)"
...
echo "== External signals ES rollout =="
...
select * from public.refresh_external_signal_rollup_es('${TENANT_ID}'::text);
SQL
  exit 0
fi
...
create temp table tmp_external_signals_es (
  tenant_id text,
  signal_type text,
  signal_source text,
...
\copy tmp_external_signals_es from '${TSV_FILE}' with (format text, delimiter E'\t');
...
with staged as (
  ...
  from tmp_external_signals_es
), upserted as (
  select public.upsert_external_signal_es(
    s.tenant_id,
    s.signal_type,
    s.signal_source,
    s.source_event_id,
