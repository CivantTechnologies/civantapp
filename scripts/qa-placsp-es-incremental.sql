-- QA: PLACSP ES incremental connector
-- Usage:
--   psql "$SUPABASE_DB_URL" -v tenant_id='civant_default' -f scripts/qa-placsp-es-incremental.sql

\echo '== QA: PLACSP ES incremental =='
...
from public."TendersCurrent"
where tender_id like 'PLACSP_ES:%'
group by 1
...
where tenant_id = :'tenant_id'
  and tender_id like 'PLACSP_ES:%'
group by 1,2
having count(*) > 1
order by c desc
limit 25;

\echo '== Sample latest-window 25 =='
select
  tender_id,
  published_at,
  data->>'title' as title,
  ...
  coalesce(data->>'source_url', data->>'feed_url') as source_url,
